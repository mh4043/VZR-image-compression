...
#SBATCH --job-name=openMP_compress_32
#SBATCH --output=log_vc_compress_32.log
#SBATCH --time=00:30:00
#SBATCH --constraint=AMD         #? vozlišča z AMD procesorji
#SBATCH --ntasks=1               #? ena instanca programa
#SBATCH --cpus-per-task=32       #? koliko proc jeder dodelimo nalogi --> ce to zelimo izkoristit mora bit OMP_NUM_THREADS enaka stevilka
#SBATCH --wait
...
export OMP_NUM_THREADS=32    #? koliko niti bi želeli uporabit, lahko se znotraj pograma nastavi, ampak pol treba ponovno prevajat, tu pa z okoljsko spremenljivko zamenjamo
